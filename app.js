...
const aiWorkerUrl = "https://maxiel-ganteng.shamiunaje.workers.dev/api/ai"
const fromScratchAIUrl = "https://api.fromscratch.web.id/v1/api/ai/ishchat"

function getAIProvider() {
  const provider = localStorage.getItem(aiProviderKey)
  return provider === "fromscratch" ? "fromscratch" : "publicai"
}

function getAIProviderLabel(provider = getAIProvider()) {
  return provider === "fromscratch" ? "GPT-OSS 120B" : "MAXIEL AI"
}
...
function syncAIProviderUI() {
  ...
  const name = getWebIdentityName()
  if (aiTyping) {
    aiTyping.innerHTML = `<span></span><span></span><span></span> ${provider === "fromscratch" ? "GPT-OSS 120B" : name + " AI"} sedang mengetik...`
  }
}

function setAIProvider(provider) {
  const safeProvider = provider === "fromscratch" ? "fromscratch" : "publicai"
...
resetAIProvider?.addEventListener("click", () => {
  localStorage.removeItem(aiProviderKey)
  syncAIProviderUI()
  toast("AI respons kembali ke Maxiel AI.")
})
...
async function askMaxielAI(query){
  const provider = getAIProvider()
  const url = provider === "fromscratch"
    ? `${fromScratchAIUrl}?query=${encodeURIComponent(query)}&model=${encodeURIComponent("gpt-oss-120b")}`
    : `${aiWorkerUrl}?query=${encodeURIComponent(query)}`
