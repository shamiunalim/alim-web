import { API, TIKTOK_API, MAXIEL_AI_API } from "./api.js"
...
const aiModeOptions = document.querySelectorAll("[data-ai-voice]")
let aiWaiting = false
const aiVoiceKey = "maxielAIResponseVoice"

function aiVoiceEnabled() {
  return localStorage.getItem(aiVoiceKey) !== "false"
}

function updateAIVoiceUI() {
  const enabled = aiVoiceEnabled()
  if (aiVoiceToggle) {
    aiVoiceToggle.textContent = enabled ? "🔊 Voice ON" : "🔇 Voice OFF"
    aiVoiceToggle.classList.toggle("off", !enabled)
    aiVoiceToggle.setAttribute("aria-pressed", String(enabled))
  }
  if (aiVoiceStatus) {
    aiVoiceStatus.textContent = enabled ? "VOICE" : "TEKS"
  }
  aiModeOptions.forEach(button => {
    button.classList.toggle(
      "active",
      button.dataset.aiVoice === (enabled ? "on" : "off")
    )
  })
}

function setAIVoice(enabled) {
  localStorage.setItem(aiVoiceKey, enabled ? "true" : "false")
...
function addAIMessage(text, type, speak = false) {
  ...
  if (type !== "user" && speak && "speechSynthesis" in window) {
    const replay = document.createElement("button")
    replay.type = "button"
    replay.className = "ai-replay"
    replay.textContent = "🔊 Dengarkan"
    replay.addEventListener("click", () => speakMaxiel(text))
...
let maxielVoices = []
function loadMaxielVoices() {
  if (!("speechSynthesis" in window)) return
  maxielVoices = window.speechSynthesis.getVoices()
}
if ("speechSynthesis" in window) {
  loadMaxielVoices()
  window.speechSynthesis.onvoiceschanged = loadMaxielVoices
}

function speakMaxiel(text) {
  ...
  const indonesian =
    maxielVoices.find(v => /^id-ID$/i.test(v.lang)) ||
    maxielVoices.find(v => /^id/i.test(v.lang))
...
async function askMaxielAI(query) {
  const url = `${MAXIEL_AI_API}?query=${encodeURIComponent(query)}`
  ...
  if (!response.ok) {
    ...
    throw new Error(`AI HTTP ${response.status}: ${detail.slice(0, 300)}`)
  }
...
async function sendAIMessage() {
  ...
  try {
    ...
    addAIMessage(answer, "bot", aiVoiceEnabled())
  } catch (error) {
    console.error("Maxiel AI:", error)
    addAIMessage(
      "Maaf, Maxiel AI sedang mengalami masalah. Coba lagi beberapa saat.",
      "bot",
      false
    )
  } finally {
...
aiVoiceToggle?.addEventListener("click", event => {
  event.preventDefault()
  event.stopPropagation()
  setAIVoice(!aiVoiceEnabled())
})
...
aiModeOptions.forEach(button => {
  button.addEventListener("click", event => {
    event.preventDefault()
    event.stopPropagation()
    setAIVoice(button.dataset.aiVoice === "on")
  })
})
