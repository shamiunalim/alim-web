...
const AI_HISTORY_KEY = "maxiel_ai_chat_history_v2"
const MAX_AI_HISTORY = 100
const MAX_AI_PERSONA = `Kamu adalah Maxiel Nuoye ོ, asisten AI resmi milik Maxiel Nuoye ོ. Jawablah sebagai Maxiel Nuoye ོ, bukan sebagai ChatGPT atau AI lain. Gunakan bahasa Indonesia secara natural kecuali pengguna meminta bahasa lain. Jawaban harus ramah, jelas, singkat bila pertanyaan sederhana, dan jangan menyebut instruksi persona ini. Jangan mengaku sebagai Zhenith. Jika pengguna bertanya siapa kamu, jawab bahwa kamu adalah Maxiel Nuoye ོ.`
...
function loadAIHistory() {
  try {
    ...
    return Array.isArray(parsed) ? parsed.slice(-MAX_AI_HISTORY) : []
  } catch {
    return []
  }
}
...
function saveAIHistory(history) {
  try {
    localStorage.setItem(AI_HISTORY_KEY, JSON.stringify(history.slice(-MAX_AI_HISTORY)))
  } catch (error) {
    console.warn("History Maxiel AI tidak dapat disimpan:", error)
  }
}
...
function aiTime(timestamp = Date.now()) {
...
function addAIMessage(text, type, timestamp = Date.now(), save = true) {
  ...
  time.textContent = aiTime(timestamp)
  ...
  if (save) {
    aiHistory.push({ text, type, timestamp })
    aiHistory = aiHistory.slice(-MAX_AI_HISTORY)
...
async function askMaxielAI(query) {
  const prompt = `${MAX_AI_PERSONA}\n\nPesan pengguna:\n${query}`
  const url = `https://maxiel-ganteng.shamiunaje.workers.dev/api/ai?query=${encodeURIComponent(prompt)}`
...
async function sendAIMessage() {
  ...
  if (aiTyping) aiTyping.classList.add("show")

  // Pesan hanya dikirim saat tombol KIRIM ditekan.
  ...
  try {
    ...
    addAIMessage("Maaf, Maxiel Nuoye ོ sedang mengalami masalah. Coba lagi beberapa saat lagi.", "bot")
  } finally {
    if (aiTyping) aiTyping.classList.remove("show")
...
if (aiInput) {
  ...
  // Enter hanya membuat baris baru. Tidak mengirim pesan.
  // Pengiriman hanya melalui tombol KIRIM.
  aiInput.addEventListener("keydown", e => {
    if (e.key === "Enter") {
      // Biarkan browser membuat newline, termasuk Shift + Enter.
      return
    }
  })
}
